...
namespace Services
{
        ...
        where T : class
    {
        ...
        public IEnumerable<ShapedEntity> ShapeData(IEnumerable<T> entity, string fieldsString)
        {
           var requiredFields = GetRequiredFields(fieldsString);
            return FetchData(entity, requiredFields);
        }
        ...
        private IEnumerable<PropertyInfo> GetRequiredFields(string fieldsString)
        {
            ...
            if(!string.IsNullOrEmpty(fieldsString))
            {
                ...
                foreach( var field in fields)
                {
                    ...
                    if (property is null) // boş ise geç
                        continue;
                    requiredFields.Add(property); // aksi durumda ise ekle  
                }
            }
            else
            {
